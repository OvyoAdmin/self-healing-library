...
• If the FIRST (coded) locator fails →
  1) Ask an LLM (Ollama) for an alternate locator that matches the current DOM,
  2) Validate the locator in the live page (visibility/uniqueness are configurable),
  3) Persist the healed choice (healed_locators.json + audit JSONL),
...
class HealingSelenium:
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.validate_retries = int(os.getenv("HEALING_VALIDATE_RETRIES", "1"))
    ...
    def _ask_ollama_for_new_locator(self, old_locator: str, html: str) -> Optional[Dict[str, str]]:
        print(f"\n[Healing] Asking Ollama to heal locator: {old_locator}")

        # NEW: collect DOM locator library + hints
        ...
        prompt = (
            "You are a function that outputs ONLY ONE JSON object with the schema\n"
            '{"type":"xpath\\ncss\\nid\\nname","locator":"<new_locator_string>"}\n'
            "Constraints:\n"
            "- Output exactly one JSON object, no code fences, no prose.\n"
            "- Prefer stable selectors (id/name or data-test* first; else robust xpath/css).\n"
            f"- The broken locator was: {old_locator}\n"
            "- Use the following WebDriver DOM Locator Library to ensure your suggestion exists:\n"
            f"{lib_json[:4000]}\n"
            ...
            "- If nothing obvious matches, construct a robust CSS/XPath using visible texts/aria-labels/classes present in the library."
        ...
        try:
            ...
            loc = suggestion.get("locator") or suggestion.get("value") or suggestion.get("selector")
            typ = suggestion.get("type")
            if not loc or not str(loc).strip():
                raise ValueError(f"Model response missing 'locator': {suggestion}")

            if not typ:
                s = str(loc).lstrip()
                if s.startswith(("/", "(", ".//", "//")):
                    typ = "xpath"
                elif s.startswith("css=") or s.startswith("css:") or (
                    any(ch in s for ch in ["[", "]", ">", "+", "~"]) and not s.startswith("id=")
                ):
                    typ = "css"
                elif re.match(r"^[A-Za-z_][\w\-.:]*$", s) and " " not in s:
                    typ = "id"
            ...
            out = {"type": typ, "locator": str(loc).strip()}
            print(f"[Healing] ✅ Suggestion: {out}")
            return out
        except Exception as e:
    ...
    def click_element(self, locator: str):  # noqa: PLR0911
        ...
        heal_result = self._heal_locator(locator) if not healed else {"entry": healed, "rewrite_stats": None}
        if heal_result and heal_result.get("entry"):
            ...
            self._maybe_fail_post_heal(locator, new_loc, heal_result.get("rewrite_stats"))
            return
        else:
    ...
    def _record_healing(self, old_locator: str, suggestion: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        self._append_audit(old_locator, new_entry, event_type="heal")
    ...
    def _heal_locator(self, locator: str) -> Optional[Dict[str, Any]]:  # noqa: PLR0912
        ...
        { 'entry': <current_healed_entry>, 'rewrite_stats': <dict or None> }
        """
        ...
        suggestion = self._ask_ollama_for_new_locator(locator, driver.page_source)
        if suggestion and "locator" in suggestion and str(suggestion["locator"]).strip():
            # Validate suggestion exists in DOM (respect env-tuned knobs)
            ok, normalized, count = self._locator_exists_in_dom(
                suggestion,
                require_visible=self.require_visible,
                unique=self.require_unique,
                retries=self.validate_retries,
            ...
            if not ok:
                # Optional artifact when model suggests a non-working locator
                if self.capture_on_fail:
                    self._capture_page_artifacts("suggested_locator_not_in_dom", suggestion.get("locator"))
                print(f"[Healing] ❌ Suggested locator not found in DOM: {normalized} (count={count})")
                return None

            # Use the normalized, working locator form when persisting
            suggestion["locator"] = normalized
            rewrite_stats = self._record_healing(locator, suggestion)
            return {"entry": self._get_current_healed(locator), "rewrite_stats": rewrite_stats}
        return None

    def _maybe_fail_post_heal(self, old_locator: str, new_locator: str, rewrite_stats: Optional[Dict[str, Any]]):
